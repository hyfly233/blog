...
<span class="line"></span>
<span class="line"></span>
<span class="line"></span></code></pre><div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0;"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>SeaTunnel 端发送数据重复是：原先使用本地模式，并且重启任务也一直使用的上述的步骤一，导致每次都是启动的一个新的 job</p><h2 id="vm-的配置" tabindex="-1"><a class="header-anchor" href="#vm-的配置"><span>VM 的配置</span></a></h2><p><a href="https://docs.victoriametrics.com/#deduplication" target="_blank" rel="noopener noreferrer">https://docs.victoriametrics.com/#deduplication</a> VM 的启动命令行参数 &quot;-dedup.minScrapeInterval=1ms&quot; 可以解决在存储时数据重复的问题。该值为正值时，VM 会保留在该时间间隔中的最大值</p><h3 id="问题点" tabindex="-1"><a class="header-anchor" href="#问题点"><span>问题点</span></a></h3><p>VM 的配置只能从存储层解决数据重复的问题，SeaTunnel 端发送重复数据是 VM 管不了的</p><div class="language-bash line-numbers-mode" data-highlighter="prismjs" data-ext="sh" data-title="sh"><pre><code><span class="line"></span>
<span class="line"><span class="token function">sh</span> ./bin/seatunnel.sh <span class="token punctuation">\\</span></span>
