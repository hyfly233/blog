...
+ throw：用于主动抛出`java.lang.Throwable`类的一个实例化对象，意思是说可以通过关键字 throw 抛出一个`Error`或者 一个`Exception`
+ throws：作用是作为方法声明和签名的一部分，方法被抛出相应的异常以便调用者能处理。在 Java 中，任何未处理的受检查异常强制在 throws 子句中声明
