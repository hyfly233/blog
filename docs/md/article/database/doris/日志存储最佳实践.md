...
完成资源评估后，可以开始部署 Apache Doris 集群，推荐在物理机及虚拟机环境中进行部署。
...
- 使用时间字段上的 (range 分区) (`PARTITION BY RANGE(`ts`)`)，并开启 [动态分区](https://doris.apache.org/zh-CN/docs/table-design/data-partitioning/dynamic-partitioning) (`"dynamic_partition.enable" = "true"`)，按天自动管理分区。
